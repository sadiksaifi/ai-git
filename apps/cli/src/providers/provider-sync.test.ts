...
import { PROVIDERS as META_PROVIDERS } from "@ai-git/meta";
import { PROVIDERS as REGISTRY_PROVIDERS } from "./registry.ts";

/**
 * Ensures @ai-git/meta's PROVIDERS (display metadata) and the runtime
...
describe("provider sync", () => {
  ...
  test("every runtime provider has a matching @ai-git/meta entry", () => {
    for (const id of registryIds) {
      expect(metaIds.has(id)).toBe(true);
    }
  });

  test("every @ai-git/meta provider has a matching runtime registry entry", () => {
